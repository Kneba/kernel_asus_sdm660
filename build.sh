...
TZ=Asia/Jakarta
DATE=$(date '+%Y%m%d')
FINAL_KERNEL_ZIP="TheOneMemory-Kernel-X00T-$(date '+%Y%m%d-%H%M').zip"
...
sed -i "s/kernel.string=.*/kernel.string=TheOneMemory/g" anykernel.sh
sed -i "s/kernel.type=.*/kernel.type=Stock/g" anykernel.sh
sed -i "s/kernel.for=.*/kernel.for=TheOneMemory/g" anykernel.sh
...
sed -i "s/device.name3=.*/device.name3=Zenfone Max Pro M1 (X00T)/g" anykernel.sh
sed -i "s/device.name4=.*/device.name4=ASUS_X00TD/g" anykernel.sh
sed -i "s/device.name5=.*/device.name5=ASUS_X00T/g" anykernel.sh
sed -i "s/X00TD=.*/X00TD=1/g" anykernel.sh
cd META-INF/com/google/android
sed -i "s/KNAME/ZEUS/g" aroma-config
